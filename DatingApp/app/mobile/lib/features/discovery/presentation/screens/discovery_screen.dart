...
/// The discovery feed: a lazily-paginated, filterable list of public profiles.
...
class _DiscoveryScreenState extends ConsumerState<DiscoveryScreen> {
  ...
  Future<void> _openFilters() async {
    final DiscoveryFilters current = ref.read(
      discoveryFiltersControllerProvider,
    );
    final DiscoveryFilters? result = await showModalBottomSheet<DiscoveryFilters>(
      context: context,
  ...
  Widget _body(DiscoveryState state) {
    ...
    if (state.status == DiscoveryStatus.error && state.profiles.isEmpty) {
      return _Message(
        ...
        actionLabel: 'Retry',
        onAction: () => ref.read(discoveryControllerProvider.notifier).refresh(),
      );
    }
